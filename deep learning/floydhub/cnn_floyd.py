...
train_datagen = ImageDataGenerator( # will be used to augment training set
        rescale=1./255, # this brings all our pixel values (from between 0 and 255) to between 0 and 1
        shear_range=0.2, # random transvection
        zoom_range=0.2, # random zooms
        horizontal_flip=True)

test_datagen = ImageDataGenerator(rescale=1./255) # will be used to augment test set

training_set = train_datagen.flow_from_directory(directory='https://www.floydhub.com/gboyegaanu/datasets/training_set/1:train', # folder in working directory
        target_size=(64, 64), # size of images expected in the cnn model
        batch_size=32, # batch of imges processed by cnn after which weights will be calulated
        class_mode='binary')

test_set = test_datagen.flow_from_directory(
        directory='gboyegaanu/datasets/test_set/1:test',
        target_size=(64, 64),
        batch_size=32,
        class_mode='binary')
